//XXX JSE TO DO: fix ERR_LIBC's, originally designed to return strerror;
// set appropriately; set ERR_EOF where appropriate
...
Strtol(long int* ret, const char* nptr, char** endptr, int base)
{
    ...
    *ret = strtol(nptr, &e, base);
    ErrUnless(*nptr != '\0' && *e == '\0', ESTATS_ERR_STRING_CONVERSION);
    if (endptr != NULL)
        *endptr = e;
